...
/**
 * this function is called by the source-line.ts and highlight.ts
 */
function md_edit_event(target) {
  if (window.USER_IS_TOUCHING) {
    ...
    if (dateTick) {
      ...
      // overlook the first click event but show the edit-it icon to let user click
      // only for common text, not for code fence
      if (diff < 200) return;
    }
  }
  ...
  element.dispatchEvent(
    new CustomEvent("edit-it", {
      bubbles: true,
      detail: {
        sourceLine: JSON.parse(element.getAttribute("data-source-lines")),
      },
    })
  );
}
...
function runHTML() {
  ...
  if (isTrusted && !frame.hasCustomConsoleLog) {
    ...
    let consoleLogNative = frame.contentWindow.console.log.bind(
    ...
    frame.contentWindow.console.log = function (...args) {
      ...
      window.postMessage(
        { type: "console-log", log: formattedArgs, codeBoxId: elem.id },
        "*"
      );
    };
  }
